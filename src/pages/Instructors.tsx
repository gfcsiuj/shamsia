      ...
      <div className="container mx-auto px-4 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {instructors.map((instructor, index) => (
            <div key={instructor.id} className={`delay-${(index % 5) * 100}`}>
              <InstructorCard 
      ...
      {selectedInstructor && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto relative z-10 animate-scale-in">
            <button 
              onClick={() => setSelectedInstructor(null)}
              className="absolute top-4 left-4 p-2 bg-slate-100 rounded-full hover:bg-slate-200 text-slate-500 transition"
            >
              <X size={24} />
            </button>
            ...
            <div className="flex flex-col md:flex-row">
              ...
              <div className="bg-slate-50 p-8 md:w-1/3 text-center md:text-right border-b md:border-b-0 md:border-l border-slate-100">
                ...
                <p className="text-secondary-600 font-bold mb-6">{selectedInstructor.role}</p>
                
                <div className="space-y-4 text-sm text-slate-600">
                  <div className="flex items-center gap-3 justify-center md:justify-start">
                    <Mail size={18} className="text-primary-500" />
                    <span>{selectedInstructor.socials?.email || 'instructor@shamsia.edu'}</span>
                  </div>
                  <div className="flex items-center gap-3 justify-center md:justify-start">
                    <Phone size={18} className="text-primary-500" />
                    <span className="ltr">{selectedInstructor.socials?.phone || '+964 770 000 0000'}</span>
                  </div>
                </div>
                ...
                <div className="mt-8 pt-8 border-t border-slate-200">
                  ...
                  <div className="flex flex-wrap gap-2 justify-center md:justify-start">
                    <span className="px-3 py-1 bg-white border border-slate-200 rounded-full text-xs text-slate-500">Certified Trainer</span>
                    <span className="px-3 py-1 bg-white border border-slate-200 rounded-full text-xs text-slate-500">Expert</span>
                  </div>
                </div>
              </div>
