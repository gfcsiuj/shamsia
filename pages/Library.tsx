  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-24">
      ...
      <div className="container mx-auto px-4 -mt-4 relative z-20">
        ...
        <div className="bg-white rounded-[2rem] shadow-lg p-6 mb-10 flex flex-col md:flex-row gap-6 justify-between items-center animate-fade-up delay-200">
          ...
          <div className="relative w-full md:w-72">
            ...
            <input
              ...
              className={`w-full ${isEnglish ? 'pl-12 pr-4' : 'pl-4 pr-12'} py-3 rounded-xl border border-slate-200 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100 outline-none transition text-sm font-bold`}
              value={searchQuery}
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredResources.map((resource, index) => (
              <div key={resource.id} className="bg-white rounded-[2rem] shadow-sm border border-slate-100 hover:shadow-xl hover:-translate-y-2 transition-all duration-300 p-6 flex flex-col animate-fade-up" style={{ animationDelay: `${index * 100}ms` }}>
                <div className="flex justify-between items-start mb-4">
                  <div className={`w-14 h-14 rounded-xl flex items-center justify-center ${getColor(resource.type)}`}>
                    {getIcon(resource.type)}
                  </div>
                  <span className="text-xs text-slate-400 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100 font-bold">{resource.category}</span>
                </div>

                <h3 className="text-lg font-black text-slate-800 mb-2 line-clamp-2 italic">{resource.title}</h3>
                ...
                <div className="pt-4 border-t border-slate-50 flex justify-between items-center mt-auto">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-center gap-2 text-emerald-600 font-bold text-sm hover:text-emerald-700 group"
                  >
          ...
          <div className="text-center py-24 bg-white rounded-[2rem] border border-dashed border-slate-200">
            <div className="inline-flex justify-center items-center w-16 h-16 rounded-2xl bg-slate-50 mb-4 text-slate-400">
              <Book size={32} />
            </div>
