...
const CourseCard: React.FC<CourseCardProps> = ({ course, instructor: propInstructor }) => {
    ...
    return (
        <div className={`group rounded-[1.5rem] lg:rounded-[3rem] border shadow-sm overflow-hidden transition-all duration-500 hover:shadow-[0_30px_60px_-15px_rgba(16,185,129,0.2)] dark:hover:shadow-[0_30px_60px_-15px_rgba(16,185,129,0.1)] hover:-translate-y-3 cursor-pointer ${isDarkMode
            ? 'bg-slate-800 border-slate-700'
            : 'bg-white border-slate-100'
            }`}>
            {/* Image Area */}
            <div className="relative h-32 lg:h-72 overflow-hidden">
                <img
                    key={currentImgIdx}
                ...
                {/* Tag Badge */}
                <div className={`absolute top-3 lg:top-6 ${isEnglish ? 'left-3 lg:left-6' : 'right-3 lg:right-6'} px-2 py-1 lg:px-4 lg:py-2 backdrop-blur-md rounded-lg lg:rounded-2xl text-[8px] lg:text-xs font-black shadow-lg border italic ${tagStyle.text}`}>
                    {tagStyle.label}
                </div>
            ...
            {/* Content Area */}
            <div className={`p-3 lg:p-10 ${isEnglish ? 'text-left' : 'text-right'}`}>
                {/* Meta Info */}
                <div className="flex items-center gap-2 lg:gap-4 mb-2 lg:mb-4 flex-wrap">
                    <span className={`flex items-center gap-1 lg:gap-2 text-[8px] lg:text-xs font-black uppercase tracking-widest ${isDarkMode ? 'text-slate-500' : 'text-slate-400'
                        }`}>
                        <Users className="w-3 h-3 lg:w-4 lg:h-4 opacity-50" /> {course.studentsCount || 0} {t('طالب', 'Students')}
                    </span>
                    <span className="flex items-center gap-1 lg:gap-2 text-[8px] lg:text-xs font-black text-amber-500 uppercase tracking-widest">
                ...
                <Link to={`/courses/${course.id}`}>
                    <h4 className={`text-xs lg:text-2xl font-black mb-1 lg:mb-3 group-hover:text-emerald-500 transition-colors leading-snug italic tracking-tight line-clamp-2 ${isDarkMode ? 'text-white' : 'text-slate-900'
                        }`}>
                        {course.title}
                    </h4>
                </Link>
                ...
                {/* Description */}
                <p className={`hidden lg:block text-sm leading-relaxed mb-6 line-clamp-2 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                    {course.description}
                </p>

                {/* Footer: Price & Action */}
                <div className={`flex items-center justify-between pt-3 lg:pt-6 border-t ${isDarkMode ? 'border-slate-700' : 'border-slate-50'
                    }`}>
                    <div className="flex flex-col">
                        {!course.hidePrice && (
                            <>
                                <span className={`text-[8px] lg:text-[10px] font-black uppercase tracking-widest mb-0.5 lg:mb-1 italic ${isDarkMode ? 'text-slate-500' : 'text-slate-400'
                                    }`}>
                                    {t('سعر الكورس', 'Course Price')}
                                </span>
                                {Number(course.oldPrice) > 0 && (
                                    <span className={`text-[10px] lg:text-xs line-through ${isDarkMode ? 'text-slate-500 decoration-slate-600' : 'text-slate-400 decoration-slate-300'
                                        }`}>
                                        {course.oldPrice.toLocaleString()} {t('د.ع', 'IQD')}
                                    </span>
                                )}
                                <span className={`text-sm lg:text-3xl font-black tracking-tighter italic ${isDarkMode ? 'text-white' : 'text-slate-900'
                                    }`}>
                    ...
                    <Link
                        ...
                        className={`w-8 h-8 lg:w-14 lg:h-14 rounded-xl lg:rounded-2xl flex items-center justify-center group-hover:bg-emerald-600 group-hover:text-white transition-all shadow-sm group-hover:shadow-xl group-hover:rotate-6 active:scale-90 ${isDarkMode
                            ? 'bg-slate-700 text-slate-300'
                            : 'bg-slate-100 text-slate-900'
                            }`}
                    >
                        <ChevronIcon className="w-4 h-4 lg:w-7 lg:h-7" />
                    </Link>
                </div>
            </div>
        </div>
